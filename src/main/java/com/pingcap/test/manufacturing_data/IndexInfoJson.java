package com.pingcap.test.manufacturing_data;
...
public class IndexInfoJson {
    ...
    private static final Logger logger = Logger.getLogger(IndexInfoJson.class);
    ...
    public static void main(String[] args) {
        ...
        logger.info(" create test data end  A001{" + atc_a.get() + "} B001{" + atc_b.get() + "}  C001{" + atc_c.get() + "} ");
    ...
    private static class ParallelExecuteCreateDataTask extends RecursiveTask<Integer> {
        ...
        protected Integer compute() {
            if (endValue - startValue < IndexInfoJson.EACH_SLICES) {
                this.logger.info(" startValue {" + startValue + "} endValue {" + endValue + "} ");
                try {
                    long i = startValue;
                    for (; i <= endValue; i++) {
                    ...
                    e.printStackTrace();
                    this.logger.error(" create test data parallelExecuteCreateDataTask exception {} ", e);
                    throw e;
                }
                return startValue;
            }
